...
import asyncio
from Utilities.staffchecks import Anystaffcheck, leadguide, guide, admin, user_has_role
...
@commands.command(name="compileembed", description ="Use this command to be walked through a embed compiling chat, and then send it")
@Anystaffcheck((leadguide, guide, admin))
async def painfulembed(ctx, number_of_embeds: int):
...
def setup_command(cog):
    cog.bot.add_command(painfulembed)
    return painfulembed
...
class InteractiveEmbedCreation:
    ...
    async def ask_for_input(self, prompt, check_length=None, check_format=None):
        ...
        while True:
            try:
                ...
                contentl = message.content.split(" ")
                newmsg = []
                for i in contentl:
                    if i.startswith(":") and i.endswith(":"):
                        ...
                        for guild in self.ctx.bot.guilds:
                            for emoji in guild.emojis:
                                if emoji.name.lower() == i.replace(":", "").lower():
                                    if found == False:
                                        newmsg.append(f"<:{emoji.name}:{emoji.id}>")
                                        found = True
                        if found == False:
                            newmsg.append(i)
    ...
    async def create_embeds(self):
        try:
            ...
            if True:
                destination_msg = await self.ask_for_input("Please provide a channel, thread, or forum post ID or mention for sending the embeds.")
                if destination_msg is None:
                    embed_json = self.embeds[self.current_step].to_dict()
                    jstr = json.dumps(embed_json, indent=1)
                    ...
                    async with aiofiles.open(temp_file_path, 'w') as file:
                        await file.write(jstr)
                    await self.ctx.send("Destination not found or not messageable.\nHeres the JSON object so you can use `jembed` to quickly reconstruct the embed object", file=disnake.File(temp_file_path, filename="output.json"))
                    await aios.remove(temp_file_path)
                    return

                destination = None
                if destination_msg.startswith('<#') and destination_msg.endswith('>'):
                    destination_id = int(destination_msg[2:-1])
                    destination = self.bot.get_channel(destination_id)
                else:
                    try:
                        destination_id = int(destination_msg)
                        destination = self.bot.get_channel(destination_id)
                    except ValueError:
                        await self.ctx.send("Invalid ID or mention provided.")
                        destination = None

                if destination is None or not isinstance(destination, disnake.abc.Messageable):
                    embed_json = self.embeds[self.current_step].to_dict()
                    jstr = json.dumps(embed_json, indent=1)
                    temp_file = tempfile.NamedTemporaryFile(mode='w+', suffix='.txt', delete=False)
                    temp_file_path = temp_file.name
                    ...
                    async with aiofiles.open(temp_file_path, 'w') as file:
                        await file.write(jstr)
                    await self.ctx.send("Destination not found or not messageable.\nHeres the JSON object so you can use `jembed` to quickly reconstruct the embed object", file=disnake.File(temp_file_path, filename="output.txt"))
                    await aios.remove(temp_file_path)
                    return
            else:
                destination = self.ctx.bot.get_channel(1196734012845215804)


            confirm = await self.ask_for_input(f"Type `confirm` to send the embeds{'' if True else ' to pend verification by a lead guide.'}.")
            if confirm.lower() != "confirm":
                await self.ctx.send("Embed sending cancelled.")
                return

            
            for embed in self.embeds:
                await destination.send(embeds=self.embeds)

            await self.ctx.send("Embed(s) sent successfully.")
        except Exception as e:
